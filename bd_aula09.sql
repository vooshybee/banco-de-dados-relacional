...
SELECT p.nome, sum(e.quantidade)  FROM  tbl_peca p JOIN tbl_estoque e ON p.cod_peca = e.cod_peca group by p.nome;

--2 Liste os nomes das peças e a soma das suas respectivas quantidades, caso a soma das peças seja maior que 20
SELECT p.nome, sum(e.quantidade)  FROM  tbl_peca p JOIN tbl_estoque e ON p.cod_peca = e.cod_peca group by p.nome having sum(e.quantidade) > 20;

--3 Listar quantidade de fornecedores em cada cidade.

SELECT cidade, count(*) Quantidade from tbl_fornecedor group by cidade ;

--4 listar quantidade de peças de cada cor

SELECT cor, count(*) FROM tbl_peca group by cor;

--5 listar quantidade de peças de cada cor. Mostrar somente as que possuem mais de 1 peça.

SELECT cor, count(*) FROM tbl_peca group by cor having count(*) > 1;
