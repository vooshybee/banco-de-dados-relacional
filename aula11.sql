...
CREATE PROCEDURE proc_upd_nome_depart (codigo int, novo_nome text)
language plpgsql
AS
$$
BEGIN
...
CREATE PROCEDURE proc_delprojeto (cod_projetos int)
language plpgsql
AS
$$
BEGIN
DELETE FROM tbl_projetos WHERE cod_projeto = cod_projetos;
END
$$;
...
CREATE PROCEDURE proc_projeto_arquivado (cod_projetos int)
language plpgsql
AS
$$
BEGIN
    ...
    SELECT cod_projeto,nome FROM tbl_projetos WHERE cod_projeto = cod_projetos;
    DELETE FROM tbl_projetos WHERE cod_projeto = cod_projetos;
END
$$;