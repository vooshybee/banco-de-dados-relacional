...
BEGIN resultado:= valor1 + valor2; RETURN resultado; 
END $$;
SELECT * FROM fc_soma('valor1','valor2')
...
CREATE FUNCTION fc_maior_salario(OUT maximo real) 
language plpgsql 
AS $$
BEGIN 
SELECT salario INTO maximo FROM tbl_empregados 
ORDER BY salario DESC;
END $$;
SELECT * FROM fc_maior_salario();
--3 crie a funcao fc_media_salario que retorna a media dos salarios dos empregados

CREATE FUNCTION fc_media_salario(out media real)
language plpgsql
AS $$ 
BEGIN 
SELECT avg(salario) INTO media FROM tbl_empregados; END $$;
SELECT * FROM fc_media_salario();
...
CREATE FUNCTION fc_salarios (codigo int, OUT salario_atual real, OUT aumento real, OUT desconto real)
language plpgsql
AS $$
BEGIN 
SELECT salario INTO salario_atual FROM tbl_empregados WHERE cod_empregado = codigo;
SELECT (salario * 1.10) INTO aumento FROM tbl_empregados WHERE cod_empregado = codigo;
SELECT (salario * 0.85) INTO desconto FROM tbl_empregados WHERE cod_empregado = codigo;
END $$;
SELECT * FROM fc_salarios('valor do codigo empregado');
...
BEGIN
RETURN QUERY
SELECT te.cod_projeto, te.nome FROM tbl_projetos AS te;
RETURN;
END $$;
...
CREATE FUNCTION fc_proj_departamento(codigo int, OUT quantidade_projetos int)
language plpgsql
AS $$
BEGIN
SELECT count (cod_projeto) INTO quantidade_projetos FROM tbl_projetos where cod_departamento = codigo;
END $$;
SELECT * FROM fc_proj_departamento('valor do cod_departamento');
