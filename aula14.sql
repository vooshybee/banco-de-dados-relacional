...
CREATE OR REPLACE FUNCTION fc_atualizar_proj_departamentos_deletados()
RETURNS TRIGGER
language plpgsql
AS $$
BEGIN
...
RETURN OLD;
END $$;
CREATE OR REPLACE TRIGGER tr_projeto_deletado
AFTER DELETE ON tbl_projetos 
FOR EACH ROW 
EXECUTE FUNCTION fc_atualizar_proj_departamentos_deletados();
--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado

CREATE OR REPLACE FUNCTION fc_atualizar_proj_departamentos_adicionados()
RETURNS TRIGGER
language plpgsql
AS $$
BEGIN
...
RETURN NEW;
END $$;
CREATE OR REPLACE TRIGGER tr_adicionar_projeto
AFTER INSERT ON tbl_projetos 
FOR EACH ROW 
EXECUTE FUNCTION fc_atualizar_proj_departamentos_adicionados();
...
CREATE TABLE IF NOT EXISTS tbl_empregado_historico_salario (cod_empregado int, salario numeric, data_atualizacao timestamp DEFAULT CURRENT_TIMESTAMP)

CREATE OR REPLACE FUNCTION fc_salario_atualizado()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER tr_salario_atualizado
AFTER UPDATE OF salario ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION fc_salario_atualizado();
...
CREATE OR REPLACE FUNCTION fc_atualiza_data_atualizacao()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION fc_atualiza_data_atualizacao();